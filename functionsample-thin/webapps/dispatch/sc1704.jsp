<%--
  - $Id: sc1704.jsp 11645 2006-04-27 10:33:04Z bs-eunsu $
  -
  - Copyright 2006 NTT DATA Corporation.
  -
  --%>
<%@page contentType="text/html; charset=Windows-31J" language="java" pageEncoding="Windows-31J"%>

<%@ taglib uri="/struts-html" prefix="html"%>
<%@ taglib uri="/struts-bean" prefix="bean"%>
<%@ taglib uri="/struts-logic" prefix="logic"%>
<%@ taglib uri="/terasoluna" prefix="t"%>
<%@ taglib uri="/terasoluna-struts" prefix="ts"%>

<bean:define id="titleUsecaseName" scope="page" value="標準ディスパッチャ機能"/>
<bean:define id="titleScreenName" scope="page" value="ユーザ数エラー"/>
<bean:define id="screenName" scope="page" value="ユーザ数エラー"/>
<bean:define id="screenID" scope="page" value="SC1704"/>

<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01//EN" "http://www.w3.org/TR/html4/strict.dtd">
<html:html locale="true">
  <%@ include file="../sc0003.jspf" %>
  <!-- begin body-->

  <h4>エラー</h4>
  <p>
  メッセージ：<ts:errors />
  </p>
  <p>
  ※アクションフォームのユーザ情報オブジェクトの数が10個に設定されたため、データベースにユーザが10人を<br>
    超えた場合エラーが発生します。データベースのデータを修正して下さい。
  </p>
  <br>
  <hr>
  <br>
  <html:button property="menuForward" value="メニュー画面に戻る"
  onclick="location.href='../menuForward.do'" />
  <br>
  <br>
  <br>
  <!-- end body-->
  
<%@ include file="../sc0004.jspf" %>
</html:html>